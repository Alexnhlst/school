...
int socket_id; // identificiatore del socket
...
int UDP_init(unsigned short port_number)
{
    ...
    if (bind(
            socket_id, (struct sockaddr *)&add, sizeof(add)) < 0)
        {
            close(socket_id);
            return -1;
        }
             ...
             unsigned char data[], int N_byte)
{
    ...
    // trasmissione dataagram
    if ((n = sendto(socket_id, (void *)data, N_byte, 0,
...
unsigned char data[], int size) {
    ...
    if ((n = recvfrom(socket_id, (void *)data, size, 0,
    (struct sockaddr*)&add, &dim)) <=0)
        return -1;
